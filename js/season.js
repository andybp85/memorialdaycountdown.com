...
;(function () {
    ...
    const isSummer = now => {
        if (override === 'summer' || override === 'off') return override === 'summer'

        return now >= lastMondayOfMay(now.getFullYear()) && now < dayAfter(firstMondayOfSeptember(now.getFullYear()))
    }

    const PANORAMA_LABELS = {
        summer:
            'Illustration of the Asbury Park boardwalk in summer: a turning Ferris wheel,' +
            ' a beach-pass pavilion, striped umbrellas on the sand, and the sea',
        off:
            'Illustration of the Asbury Park boardwalk in the off-season: a still Ferris wheel,' +
    ...
    const applySeason = now => {
        ...
        if (root.dataset.season !== season) {
            ...
            document.querySelector('meta[name="theme-color"]').content = season === 'summer' ? '#8fd0ce' : '#101b26'
            document.querySelector('meta[name="color-scheme"]').content = season === 'summer' ? 'light' : 'dark'
            const panorama = document.querySelector('.panorama')
            if (panorama) panorama.setAttribute('aria-label', PANORAMA_LABELS[season])
        }
