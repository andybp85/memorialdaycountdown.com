...
;(function () {
    ...
    const render = () => {
        ...
        const target = summer ? dayAfter(firstMondayOfSeptember(now.getFullYear())) : nextMemorialDay(now)
        ...
        const nextDate = nextMemorialDay(now).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
        dateSpans.forEach(span => {
            if (span.textContent !== nextDate) span.textContent = nextDate
        })
    }
    ...
    const fitPanorama = () => {
        ...
        panorama.setAttribute('preserveAspectRatio', cropped || aspect > 3 ? 'xMidYMax slice' : 'xMidYMax meet')
